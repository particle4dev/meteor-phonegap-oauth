...
OAuth._endOfLoginResponse = function(res, details) {
  ...
  if (details.error) {
    ...
    console.log(content('window.location.replace("' + Meteor.absoluteUrl() + '_phoneoauth?error=' +
             (details.error instanceof Error ?
              details.error.message : details.error) + '");') + ' zzz');
    res.end(content('window.location.replace("' + Meteor.absoluteUrl() + '_phoneoauth?error=' +
             (details.error instanceof Error ?
              details.error.message : details.error) + '");'), 'utf-8');
    return;
  }
